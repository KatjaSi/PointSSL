...
from models.transformers import PCT_ml, PCT_ml2, PCT_Decoder, PCT_BASE2
...
if __name__ == '__main__':
    ...
    train_set = ModelNetForMaskedLearning(train_points, num_points=1024, rotate=False, mask=False, num_key_points=1, num_neighbors=1)
    ...
    test_set = ModelNetForMaskedLearning(test_points, num_points=1024, rotate=False, mask=False, num_key_points=1, num_neighbors=10)
    ...
    encoder = PCT_ml2(out_dim=128)#, mask=True)
   ...
   # optimizer = SGD(list(encoder.parameters()) + list(decoder.parameters()), lr=0.01, momentum=0.9)
    ...
    for epoch in range(num_epochs):
        ...
        for data in train_loader:
            ...
            reconstructed = decoder(latent).float()
    ...
    latent = encoder(x_aug.permute(0,2,1), mask=mask).float()
